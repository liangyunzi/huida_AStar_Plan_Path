...
class UnknownMapAStarPlanner:
    ...
    def __init__(self, environment, radar_range=5):
        ...
        while open_set:
            ...
            for dy, dx in self.directions:
                ...
                if prev_dir is not None and curr_dir != prev_dir:
                    turn_penalty = move_cost * 0.2  # 可调整惩罚系数
...
class UnknownMapVisualizer:
    ...
    def _plot_known_map(self, ax, title):
        """绘制已知地图"""
        # 创建已知地图可视化
        known_map = np.ones((self.env.height, self.env.width, 3)) * 0.8  # 默认灰色（未知）

        # 已知障碍物 - 黑色
        obstacle_mask = self.planner.obstacle_grid
        known_map[obstacle_mask] = [0.1, 0.1, 0.1]  # 黑色

        # 已知自由空间 - 根据地形着色
        known_free_mask = (self.planner.known_grid >= 0) & ~obstacle_mask
        for terrain_type in [TerrainType.GRASS, TerrainType.TREE, TerrainType.ROCK, TerrainType.MUD]:
            mask = (self.planner.known_terrain == terrain_type.value) & known_free_mask
            if terrain_type == TerrainType.GRASS:
                known_map[mask] = [0.8, 1.0, 0.8]  # 浅绿色
            elif terrain_type == TerrainType.TREE:
                known_map[mask] = [0.2, 0.6, 0.2]  # 深绿色
            elif terrain_type == TerrainType.ROCK:
                known_map[mask] = [0.5, 0.5, 0.5]  # 灰色
            elif terrain_type == TerrainType.MUD:
                known_map[mask] = [0.6, 0.4, 0.2]  # 棕色

        ax.imshow(known_map, extent=[0, self.env.width, 0, self.env.height],
    ...
    def animate_exploration(self, interval=50):
        ...
        def update(frame):
            ...
            if frame % 4 == 0:
                ...
                self.radar_circle2 = patches.Circle((x_pos, y_pos), self.planner.radar.max_range,
                                                    fill=False, edgecolor='red', linestyle='--',
...
def main():
    ...
    # 创建环境（使用较小地图提高演示速度）
    env = ForestEnvironmentVisualizer(width=50, height=50, seed=45)

    # 创建未知环境规划器
    planner = UnknownMapAStarPlanner(env, radar_range=5)
    ...
    print("\n🎬 开始探索动画演示...")
    anim = visualizer.animate_exploration(interval=10)
