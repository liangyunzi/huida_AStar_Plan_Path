...
class UnknownMapAStarPlanner:
    ...
    def smooth_path(self, path, max_range=3):
        """
...
class UnknownMapVisualizer:
    ...
    def __init__(self, environment, planner):
        ...
        self.radar_history_step = 2  # 每2步记录一次，减少数据量
    ...
    def _plot_known_map(self, ax, title):
        ...
        # 提取障碍物坐标：获取所有True值的(y, x)索引

        ax.imshow(self.known_map, extent=[0, self.env.width, 0, self.env.height],
                  origin='lower', alpha=0.9, zorder=1)

        obstacle_mask = self.planner.obstacle_grid
        obstacle_coords = np.argwhere(obstacle_mask)  # 返回格式：[[y1, x1], [y2, x2], ...]

        patches_list = []
        for (y, x) in obstacle_coords:
            # 绘制圆形点（转换坐标为(x, y)，因为matplotlib中x是水平轴，y是垂直轴）
            point = patches.Rectangle(
                    (x, y),  # 左下角(x, y)
                    width=1,
                    height=1,
                    color=(0, 0, 0),
    ...
    def animate_exploration(self, interval=50):
        ...
        def update(frame):
            ...
            if frame % self.radar_history_step == 0:  # 控制记录频率
            ...
            if frame % 5 == 0:
                self.ax2.clear()
                # 将路径坐标列表转换为 NumPy 数组（确保是整数类型，对应像素位置）
                y_indices = np.array(path_y, dtype=int)
                x_indices = np.array(path_x, dtype=int)
                ...
                for (hx, hy) in self.radar_history:
                    # 历史痕迹用更浅的颜色和更高透明度
                    history_circle = patches.Circle(
                        (hx, hy), self.planner.radar.max_range,
                        fill=True, facecolor='green', edgecolor='green', linestyle='--',
                        linewidth=1, alpha=0, zorder=1  # 层级低于当前雷达
                    )
                    self.ax2.add_patch(history_circle)

                # 绘制当前雷达范围（保留原样式或改为绿色，与历史区分）
...
def main():
    ...
    # 创建环境（使用较小地图提高演示速度）
    env = ForestEnvironmentVisualizer(width=60, height=60, seed=40)

    # 创建未知环境规划器
    planner = UnknownMapAStarPlanner(env, radar_range=4)
